...
def train(model: models.reinvent.Model, smiles_list: List[str], model_path: str, epochs=10, lr=0.001, patience=30000,
    ...
    for smile in smiles_list:
        if Chem.MolFromSmiles(smile) is not None:
            temp_can_smile = Chem.MolToSmiles(Chem.MolFromSmiles(smile))
            can_smiles_list.append(temp_can_smile)
    ...
    for epoch in range(epochs):
        ...
        for step, batch in tqdm(enumerate(data), total=len(data)):
            ...
            # Every FIXED steps we decrease learning rate and print some information
            fixed_steps = int(len(smiles_list)/batch_size)-1
            if step % (fixed_steps // max(1, torch.cuda.device_count())) == 0 and step != 0:
                tqdm.write(("Epoch {:3d}   step {:3d}    loss: {:5.2f}    "
                            "patience: {}    lr: {}").format(epoch, step,
                                                             loss.data,
                                                             patience,
                                                             optimizer.param_groups[0]["lr"]))
                cus_seqs, cus_likelihood, cus_ = model.sample(128, temperature=temperature)
                cus_valid = 0
                cus_novel = 0
                tqdm.write("\n\n*************Epoch {:2d}****************".format(epoch))
                cus_smiles = model.sequence_to_smiles(cus_seqs)
                
                can_list = []
                for smile in cus_smiles:
                    if Chem.MolFromSmiles(smile) is not None:
                        can_item = Chem.MolToSmiles(Chem.MolFromSmiles(smile))
                        can_list.append(can_item)
                
                for i, smile in enumerate(cus_smiles):
                    if Chem.MolFromSmiles(smile):
                        cus_valid += 1
                    if i < 5:
                        tqdm.write(smile)
                for smile in set(can_list):
                    if Chem.MolFromSmiles(smile):
                        if Chem.MolToSmiles(Chem.MolFromSmiles(smile)) not in can_smiles_list:
                            cus_novel+=1
                valid_pct = 100 * cus_valid / len(cus_seqs)
                unique_pct = 100 * len(set(can_list))/len(cus_seqs)
                novel_pct = 100* cus_novel / len(cus_seqs)
                report_loss = loss.data
                fixed_metrics.append([valid_pct,unique_pct,novel_pct,report_loss])
                tqdm.write("\n{:>4.1f}% valid SMILES".format(valid_pct))
                tqdm.write("****************************************\n")

        # Save the model after each epoch
        save_model(model, model_path, epoch, save_each_epoch)
    metrics_array = np.array(fixed_metrics)
    np.save(model_path[:-4]+'_metrics.npy',metrics_array)
